            ...
            <div className="py-6 space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 rounded-xl bg-slate-950 border border-slate-800">
                  ...
                  <p className={`text-2xl font-bold ${getScoreColor(selectedScore.overallScore)}`}>{selectedScore.overallScore}</p>
                </div>
                <div className="p-4 rounded-xl bg-slate-950 border border-slate-800">
                  <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-2">Conversion Likelihood</p>
                  <p className="text-2xl font-bold text-teal-400">{selectedScore.conversionLikelihood}%</p>
                </div>
              </div>
              <div className="space-y-3">
                ...
                <div className="p-4 rounded-lg bg-slate-800/50 border border-slate-700/50 text-xs text-slate-400 space-y-2">
                  {(() => {
                    let issues: string[] = []
                    try { issues = selectedScore.issuesJson ? JSON.parse(selectedScore.issuesJson) : [] } catch { /* ignore */ }
                    return issues.length > 0
                      ? issues.map((issue, i) => <p key={i}>• {issue}</p>)
                      : <p>No specific issues recorded for this scoring pass.</p>
                  })()}
                </div>
              </div>
            </div>
