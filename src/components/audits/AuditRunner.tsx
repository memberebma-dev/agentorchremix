...
export function AuditRunner({ onAuditComplete }: AuditRunnerProps) {
  ...
  const getSelectedLeadNames = () => {
    if (mode === 'single') {
      const lead = leads.find(l => l.id === selectedLeadId)
      return (lead as any)?.business_name || (lead as any)?.companyName || 'Select a lead'
    }
    ...
    <div className="audit-runner">
      ...
      {results.length > 0 && !isRunning && (
        <div className="results-summary">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
            ...
            {selectedReport && (
              <button 
                ...
                style={{ background: 'transparent', border: '1px solid #134E4A', color: '#5EEAD4', padding: '4px 8px', borderRadius: '4px', cursor: 'pointer', fontSize: '0.8rem' }}
              >
                ← Back to List
              </button>
            )}
          ...
          {selectedReport ? (
            <AuditReportView report={selectedReport} />
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {results.map((report, index) => (
                <div 
                  ...
                  style={{ cursor: 'pointer', padding: '12px', background: 'rgba(19, 78, 74, 0.15)', borderRadius: '8px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', border: '1px solid transparent', transition: 'all 0.2s' }}
                  onMouseOver={(e) => e.currentTarget.style.borderColor = '#0D9488'}
                  onMouseOut={(e) => e.currentTarget.style.borderColor = 'transparent'}
                >
                  <div style={{ display: 'flex', flexDirection: 'column' }}>
                    <span style={{ fontWeight: 500 }}>{report.websiteUrl || 'N/A'}</span>
                    <span style={{ fontSize: '0.75rem', color: '#5EEAD4' }}>{new Date(report.generatedAt).toLocaleString()}</span>
                  </div>
                  <span className={`result-score ${report.overallScore >= 70 ? 'score-high' : report.overallScore >= 40 ? 'score-medium' : 'score-low'}`} style={{ fontWeight: 700, fontSize: '1.1rem' }}>
